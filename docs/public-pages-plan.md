...
- **End of body** (inline): in spa mode, inject the expo-reset `<style>`, the SPA CSS `<link>`, `<div id="root">` and the SPA entry `<script>`. Since Stage B, the build inlines Expo's small global CSS into `dist/index.html` as `<style data-href=…>` (it was render-blocking, ~160 ms), so the handoff copies those `<style>` blocks rather than a `<link>`. The URLs are read from `dist/index.html` at build time and written to `functions/_generated/assets.ts`. Expo Router then renders `id/[slug].tsx` (member actions, own profile, blocked or unblock) exactly as today. The island exits when `html.spa` is set.
